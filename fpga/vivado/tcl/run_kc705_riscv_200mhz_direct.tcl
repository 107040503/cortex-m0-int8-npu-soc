...
set experiment_name kc705_riscv_200mhz
...
set_property verilog_define [list FPGA_USE_AXI_BRAM FPGA_SYNTH_BRAM KC705_ENABLE_ILA] [current_fileset]

set src_files [concat \
    [list [repo_file fpga/rtl/fpga_kc705_riscv_top.v]] \
    [list [repo_file rtl/axi_bram.v]] \
    [read_filelist [repo_file rtl/filelist_with_cpu.f]] \
]
foreach f $src_files {
    if {![file exists $f]} {
...
set mem_file [repo_file fpga/vivado/mem/picorv32_npu_demo.mem]
...
set_property generic "SRAM_INIT_FILE=$mem_file FPGA_START_DELAY_CYCLES=32'h00000000" [current_fileset]

synth_design -top fpga_kc705_riscv_top -part xc7k325tffg900-2 -retiming -directive $synth_directive
...
if {$stop_after eq "synth"} {
    ...
    puts "RISC-V direct-flow stopped after synth by FPGA_STOP_AFTER."
    close_project
    return
}
...
if {$stop_after eq "implement_debug_core"} {
    ...
    puts "RISC-V direct-flow stopped after implement_debug_core by FPGA_STOP_AFTER."
    close_project
    return
}
...
puts "RISC-V direct-flow reports written to $reports_dir"
close_project
