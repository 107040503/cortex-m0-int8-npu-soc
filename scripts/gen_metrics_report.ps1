...
$defaultLogNames = @(
    ...
    "tb_hetero_soc.log",
    "tb_npu_stress.log",
    "tb_cortex_m0_cpu_npu.log"
)
$logs = Get-ChildItem sim -Filter "*.log" |
    Where-Object { $defaultLogNames -contains $_.Name } |
    Sort-Object Name
$passLines = @()
...
$requiredCover = @(
    ...
    "actual_cortex_m0_ahb_config",
    "actual_cortex_m0_ahb_to_axil_bridge",
    "actual_cortex_m0_cpu_npu_poll",
    "actual_cortex_m0_npu_irq",
    "actual_cortex_m0_zero_copy_addresses",
    "actual_cortex_m0_designstart_firmware",
    "cortex_m0_peak_over_1tops",
    "cortex_m0_bus_util_over_80"
)
...
$requiredPaths = @(
    ...
    "cortex_m0_ahb_write_npu_regs",
    "cortex_m0_ahb_read_npu_status",
    "cortex_m0_ahb_to_axi_lite_bridge",
    "cortex_m0_shared_interconnect_cpu_to_npu",
    "cortex_m0_shared_interconnect_npu_dma_to_ram"
)

$hit = 0
foreach ($point in $requiredCover) {
    if ($uniqueCover -contains $point) {
        $hit++
    }
}
...
$report.Add("# Metrics Report")
$report.Add("")
$report.Add("Generated from Icarus Verilog logs in ``sim/``.")
$report.Add("")
...
if ($failLines.Count -eq 0) {
    $report.Add("- No FAIL lines found in current logs.")
} else {
...
$report.Add("- Cortex-M0 integration evidence: official DesignStart CORTEXM0INTEGRATION executes Thumb firmware, performs AHB-Lite writes/reads, drives the AHB-to-AXI-Lite bridge, polls IRQ/done, and passes zero-copy addresses")
...
[System.IO.File]::WriteAllText((Join-Path $root "docs/metrics_report.md"), ($report -join [Environment]::NewLine), [System.Text.Encoding]::UTF8)
Write-Host "Generated docs/metrics_report.md"
