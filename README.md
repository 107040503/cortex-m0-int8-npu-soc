...
| RISC-V/PicoRV32 | RTL 回归 PASS；PicoRV32 固件通过 AXI/MMIO 配置 NPU；Vivado KC705 200MHz 实现 WNS 为正；ILA `.ltx` 已生成 | PicoRV32 RTL 开源可控，可关闭乘除法/计数器并启用 two-cycle ALU/compare，控制面更轻量，适合作为当前 200MHz主线 |
